...
const loginTpv = async (req, res) => {
	...
	try {
		...
		if (validPassword) {
			...
			let newRefreshTokenArray = !cookies?.jwt_tpv
				? foundUser.refreshToken
				: foundUser.refreshToken.filter((rt) => rt !== cookies.jwt_tpv);
			...
			if (cookies?.jwt_tpv) {
				...
				const refreshToken = cookies.jwt_tpv;
			...
			res.cookie('jwt_tpv', newRefreshToken, {
				httpOnly: true,
				secure: true,
				sameSite: 'None',
...
const refreshTpv = async (req, res) => {
	try {
		const cookies = req.cookies;
...
const logoutTpv = async (req, res) => {
	try {
		// On client, also delete the accessToken

		const cookies = req.cookies;
		if (!cookies?.jwt_tpv) return res.sendStatus(204); // No content
		const refreshToken = cookies.jwt_tpv;
